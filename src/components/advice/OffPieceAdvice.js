...
export function getOffPieceAdvice(persoObj) {
    ...
    if (fullSetKey) {
        ...
        return {
            ...
            data: { offPiece: worstPiece, avgScore: avgSetScore, is5of5: true, offPieceScore: worstPiece.score || 0 }
        };
    }
    ...
    const offPieceScore = offPiece.score || 0;

    if (offPieceScore > avgSetScore) {
        type = "success";
        msg = t('advice.offPiece.good', t('artifact.' + offPiece.type));
    } else if (isHardMainStat && offPieceScore > (avgSetScore * 0.8)) {
        type = "warning";
    ...
    return {
        type: type,
        msg: msg,
        data: { offPiece: offPiece, avgScore: avgSetScore, is5of5: false, offPieceScore }
    };
}
...
export function renderOffPieceAdvice(persoObj) {
    ...
    if (offPieceAdvice.data) {
        const { offPiece, avgScore, is5of5, offPieceScore } = offPieceAdvice.data;
        const maxScale = Math.max(offPieceScore || 0, avgScore || 0, 55);
